...
def test_message():
    lines = io.StringIO(
        "2017-07-22 20:56:39.123456 :Cassie!~abc@sdf.dkf.com PRIVMSG #chan :what the fuck\r\n"
    )
...
def test_nda_message():
    lines = io.StringIO(
        "2016-02-03 00:02:46.188104 Sending ^^ Sonic the Hedgehog (@sonic_hedgehog): Happy Hedgehog Day! https://t.co/AHkbFg74oo to #chan"
    )
    ...
    assert (
        quote.message
        == "^^ Sonic the Hedgehog (@sonic_hedgehog): Happy Hedgehog Day! https://t.co/AHkbFg74oo"
    )
    assert quote.quote_type == QuoteType.message
...
def test_join():
    lines = io.StringIO(
        "2017-07-22 20:56:39.123456 :Cassie!~abc@sdf.dkf.com JOIN #chan"
    )
    reader = NdaLogReader("#chan", "")
    quote = next(reader.read(lines))
...
def test_ban(mode, message):
    lines = io.StringIO(
        "2017-07-22 20:56:39.123456 :Cassie!~abc@sdf.dkf.com MODE #chan %s" % mode
    )
    reader = NdaLogReader("#chan", "")
    quote = next(reader.read(lines))
...
def test_ban_different_channel():
    lines = io.StringIO(
        "2017-07-22 20:56:39.123456 :Cassie!~abc@sdf.dkf.com MODE #chan +b anyname!*@*"
    )
    reader = NdaLogReader("#notchan", "")
    assert not list(reader.read(lines))
...
def test_kick():
    lines = io.StringIO(
        "2017-07-22 20:56:39.123456 :Cassie!~abc@sdf.dkf.com KICK #chan anyname :fuck off"
    )
    reader = NdaLogReader("#chan", "")
    quote = next(reader.read(lines))
